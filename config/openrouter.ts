...
class OpenRouterClient {
    ...
    private async callModel(
        ...
        if (!rawText) throw new Error('No content in NvidiaAPI response');
...
6. FIT 720px AT NATURAL SIZE — NEVER GET ZOOMED OUT (critical): The renderer measures your slide's real height and SHRINKS the whole slide (tiny fonts) if it exceeds 720px. Design so content fits at natural size; if it doesn't fit, REMOVE data. Root <section> is fixed: style='width:1440px;height:720px;box-sizing:border-box;padding:44px 64px;display:flex;flex-direction:column;gap:20px;overflow:hidden;'. Zones budget: header ≤120px, body ≤450px, footer ≤42px. The BODY must be style='flex:1;min-height:0;overflow:hidden;...' and any image/table column also needs min-height:0 — otherwise a tall child pushes past 720 and everything zooms out.
7. IMAGES ARE BOUNDED SIDE ELEMENTS: An image goes ONLY in a ~40% side column of a 2-column body, with the <img> at width:100%;height:100%;max-height:440px;object-fit:cover. NEVER a full-width/centered square image, and NEVER an image stacked under the headline (that overflows → zoom-out).
8. CONCISE ON-SCREEN DATA: On-screen text is skeletal — headline ≤ 8 words, max ~4 rows / 3 cards / 4 bars per component, no paragraphs; the depth lives in the narration. Keep body/label text ≥ 15px so it stays readable.
9. USE RICH VARIED COMPONENTS: Build the body from ONE well-crafted component — comparison table, before/after diff, progress bars, bubble cards, feature rows, accent-spine card, timeline, stat row, donut ring, definition card, stepper, chip cloud, concept-vs-example, mini bar-chart, principle band, or metric callout. Each slide uses a DIFFERENT one; consecutive slides look distinct. Rounded, soft, layered styling reads premium; avoid uniform bordered tile grids.
...
2. NO OVERLAP + NEVER ZOOM OUT: Content must NEVER overlap — every block in its own flex/grid space; headline printed once; images ONLY in a bounded ~40% side column (img max-height:440px), never a big square, never stacked under the headline. Root <section> is fixed 1440x720 (padding:44px 64px; overflow:hidden); the body is flex:1;min-height:0;overflow:hidden so nothing pushes past 720px and shrinks the fonts. Use ONE primary component (table, diff, progress bars, bubbles, feature rows, timeline, stat row, donut, definition card, stepper, chip cloud, bar-chart, metric callout) + a short headline. On-screen text stays skeletal (≤4 rows, no paragraphs, ≥15px). A code block, if used, is the ONLY primary component.
