    ...
    try {
        ...
        try {
            slidePlan = await openrouter.text(PLAN_SLIDE_PROMPT, JSON.stringify(slideContext), {
                ...
                // Compact scaffold only — thinking is ON here so tokens are slow, and
                // this route shares ONE 300s budget across both phases. Phase 2 writes
                // the full 3500-4500 word narration with thinking OFF.
                maxTokens: 4000,
                timeoutMs: 100000,
            });
        } catch (e: any) {
