...
async function upload() {
    ...
    for (let idx = 0; idx < pairs.length; idx++) {
        const { endpoint, projectId: appProjectId, apiKey, bucketId, name } = pairs[idx];
        console.log(`\n🔑 [Appwrite Key Rotation] Attempt ${idx + 1}/${pairs.length} using ${name} project ID: ${appProjectId}...`);
        ...
        try {
            ...
            console.warn(`⚠️  Upload failed with ${name} project ID ${appProjectId}: ${error?.message ?? error}`);
